...
const ProductItem = ({ product, id, updateProduct, deleteProduct }) => {
  ...
  const submitEdit = async (e) => {
    ...
    await updateProduct(
      {
        ...editedProduct,
        pro_price: Number(editedProduct.pro_price),
        pro_qty: Number(editedProduct.pro_qty),
      },
      id
    );
    setLoading(false);
    ...
    <>
      <tr className="bg-white relative">
        ...
        <td className="px-4 py-2">
          <button onClick={openModal} className="text-teal-500" disabled={loading}>
            <FaPen />
          </button>
          <button
        ...
        {loading && (
          ...
          >
            <HashLoader color="#0a7bff" size={32} />
          </td>
        )}
      </tr>
        ...
        >
          {loading ? (
            <div className="flex flex-col items-center justify-center min-h-[200px]">
              <HashLoader color="#0a7bff" size={40} />
